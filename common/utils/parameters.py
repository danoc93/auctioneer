...
def get_boolean_parameter(parameters, field, required=False, default=False):
    ...
    if parameter is None:
        return default
    if not isinstance(parameter, bool) and parameter not in ['true', 'false']:
        raise SuspiciousOperation('Parameter "{}" is not a valid boolean'.format(field))
    return parameter if isinstance(parameter, bool) else parameter == 'true'
...
def get_number_parameter(parameters, field, type=int, required=False, default=None):
    try:
